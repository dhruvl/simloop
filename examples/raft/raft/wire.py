...
Every RPC opens a fresh connection, sends one JSON object, reads one JSON
object back, and closes. One connection per call keeps request/response
correlation trivial when a timeout abandons a call whose reply is still
...
async def call(
    ...
    because the caller's only recourse is the same either way: try again
    later. Under simulation a partition stalls silently and a crashed peer
